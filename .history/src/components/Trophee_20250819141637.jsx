...
export default function Trophee() {
  ...
  const fullText ="Le trophée du meilleur établissement a été remporté par l'Institut La Lumière cette année";

  // Détecte quand la section est visible à l'écran
  useEffect(() => {
...
useEffect(() => {
  if (visible) {
    ...
    const interval = setInterval(() => {
      if (index < fullText.length) {
        ...
        clearInterval(interval); // On arrête proprement
      }
    }, 40); // vitesse d’écriture
  ...
  return (
    <section className="trophee-section" ref={textRef}>
      <div className={`trophee-container ${visible ? "visible" : ""}`}>
        <span className="trophee-icon">
          <svg
            height="200"
            width="200"
            viewBox="0 0 36 36"
        ...
        <span className="trophee-text">{displayText}</span>
      </div>
    </section>
  );
}
